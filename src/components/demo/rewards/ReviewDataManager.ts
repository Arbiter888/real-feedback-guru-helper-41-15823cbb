...
export const saveReviewData = async (
  ...
  const { data: reviewData_, error: reviewError } = await supabase
    ...
    .upsert({
      ...
      receipt_data: reviewData.analysisResult, // Keep using receipt_data for reviews table
  ...
  // Create or update email contact with receipt analysis in metadata
  const metadata: ReviewMetadata = {
    initial_review: reviewData.reviewText,
    refined_review: reviewData.refinedReview,
    receipt_analysis: reviewData.analysisResult, // Use receipt_analysis for email metadata
    server_name: reviewData.serverName,
